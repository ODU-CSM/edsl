...
class DiscriminativeLoss(nn.Module):
    ...
    def forward(self, prediction=None, target=None, weight=None, n_clusters=2):
        return self._discriminative_loss(prediction, target, n_clusters)

    def _discriminative_loss(self, prediction, target, n_clusters):
        bs, n_features = prediction.size()
        max_n_clusters = target.size(1)

        prediction = prediction.contiguous().view(bs, n_features, height * width)
        target = target.contiguous().view(bs, max_n_clusters, height * width)

        c_means = self._cluster_means(prediction, target, n_clusters)
        l_var = self._variance_term(prediction, target, c_means, n_clusters)
    ...
    def _cluster_means(self, prediction, target, n_clusters):
        bs, n_features, n_loc = prediction.size()
        max_n_clusters = target.size(1)

        # bs, n_features, max_n_clusters, n_loc
        prediction = prediction.unsqueeze(2).expand(bs, n_features, max_n_clusters, n_loc)
        ...
        for i in range(bs):
            ...
            prediction_sample = prediction[i, :, :n_clusters[i]]
            # 1, n_clusters, n_loc,
            target_sample = target[i, :, :n_clusters[i]]
            # n_features, n_cluster
            mean_sample = prediction_sample.sum(2) / target_sample.sum(2)
    ...
    def _variance_term(self, prediction, target, c_means, n_clusters):
        bs, n_features, n_loc = prediction.size()
        max_n_clusters = target.size(1)
        ...
        var = (torch.clamp(torch.norm((prediction - c_means), self.norm, 1) -
